...
class ClassifierTrajDataset(GTTrajLevelDataset):
    ...
    def prebuilt(self, agents, min_length, run):
        ...
        for traj in self.trajs:
            ...
            segments = []
            for i in range(len(obs) - self.steps + 1):
    ...
    def sample(self, num_samples, include_action=False):
        ...
        for _ in range(num_samples):
            ...
            if include_action:
                D.append((
                    np.concatenate((x_segment, x_traj[2][x_ptr:x_ptr+self.steps]), axis=1),
                    np.concatenate((y_segment, y_traj[2][y_ptr:y_ptr+self.steps]), axis=1),
